...
style_SAR_CCDR <- function(textsize=7, family="Franklin Gothic Book", family.bold = "Franklin Gothic Medium", is.cmyk = FALSE) {
  modifyList(style_base(textsize), listy(
    ...
    colors = listy(
      neutral                  = "grey50",
      neutral.light            = "grey70",
      neutral.dark             = "grey30",
      text                     = "grey20",
      text.inverse             = "white",
      blue                     = if (!is.cmyk) "#249AD6"               else cmyk(83, 28, 0, 16, maxColorValue = 100),
      blue.light               = if (!is.cmyk) "#A9DBF5"               else cmyk(31, 11, 0, 4, maxColorValue = 100),
      orange                   = if (!is.cmyk) "#DC8453"               else cmyk(0, 40, 62, 14, maxColorValue = 100),
      ...
      regions = c(
        EAS                    = if (!is.cmyk) "#2078B6"               else cmyk(80, 40, 0, 10, maxColorValue = 100),
        ECS                    = if (!is.cmyk) "#DF7F2E"               else cmyk(0, 55, 90, 10, maxColorValue = 100),
        ...
        SAS                    = if (!is.cmyk) "#CE1249"               else cmyk(2.7, 100, 58.6, 12.2, maxColorValue = 100),
    ...
    theme = function() {
      theme_minimal() +
        theme(text                 = element_text(family = family, size=textsize, color=colors$text),
              ...
              plot.caption         = element_text(hjust=0, size=rel(6/7), lineheight = 1, margin=margin(1.5,0,0,0, unit="line"), color= if (is.cmyk) "grey50" else "black"),
...
#' @export
style_SAR_CCDR_cmyk <- purrr::partial(style_SAR_CCDR, is.cmyk=TRUE)


#' @export
style_SAR_CCDR_open <- function(textsize=7, ...) {
  style_SAR_CCDR(textsize=textsize, family="Nunito Sans", family.bold = "Nunito Sans", ...)
}
