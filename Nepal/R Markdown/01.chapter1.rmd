...
fig_ch1_poverty <- function() {
  ...
  figure(
    ...
    aspect_ratio = 1,
    title = "Nepal's poverty compared with South Asia and lower-middle-income group",
...
fig_ch1_natural_disaster_frequency <- function() {
  ...
  # data
  df <- read_excel("../data/emdat_public_2022_03_14_Nepal.xlsx", skip=6) %>%
    filter(Year >= 2000) %>%
  ...
  figure(
    ...
    plot = function(df, style = style_SAR_CCDR()) {
      ...
      df.category <- df.category %>%
        ...
        arrange(category == "Climatological")
      ...
      df.category$vjust <- c(0.5, 0.5, 0.6, 0.9, 0.1)
      ...
      df.type$hjust <- c(0, 1, 1, 1, 1, 1, 1, 1)
...
fig_ch1_natural_disaster_deaths <- function() {
  ...
  figure(
    ...
    plot = function(df, style = style_SAR_CCDR()) {
      ...
      df.category$vjust <- c(0.4, 0.5, 0.9, -0.2, 0.5)
      ...
      df.type$angle <- ifelse(df.type$labelPosition/max(df.type$ymax)*360 < 180, 
                              ...
                              df.type$labelPosition/max(df.type$ymax)*360*-1 + 270)
      df.type$label <- paste0(df.type$type)
      df.type$label <- c("Earthquake", "Flood", "Landslide", "Epidemic", "Extrm temp", "Storm", "", "")
      df.type$hjust <- c(0, 1, 1, 1, 1, 1, 1, 1)
      ...
      geom_text(data = df.category, x=2.7, aes(y = labelPosition, label = label, vjust=vjust, family=style$family), size=3) +
      geom_text(data = df.type, x=4.1, aes(y = labelPosition, label = label, angle=angle, hjust=hjust, family=style$family), size=2.5, show.legend = F) + 
      coord_polar(theta="y") +
      xlim(c(1, 4.5)) +
      scale_fill_manual(values = c(
  ...
  figure(
    ...
    plot = function(df, style = style_SAR_CCDR()) {
      ...
      df.type$angle <- ifelse(df.type$labelPosition/max(df.type$ymax)*360 < 180,
                              ...
                              df.type$labelPosition/max(df.type$ymax)*360*-1 + 270)
      df.type$label <- paste0(df.type$type)
      df.type$label <- c("Earthquake", "Flood", "", "", "", "", "", "")
      df.type$hjust <- c(0, 1, 1, 1, 1, 1, 1, 1)
...
Data source: https://www.climatewatchdata.org/ghg-emissions?breakBy=sector&chartType=area&end_year=2018&regions=NPL&sectors=total-including-lucf&start_year=1990
...
fig_ch1_greenhouse_gas_emissions <- function() {
  ...
  figure(
    ...
    plot = function(df, style = style_SAR_CCDR()) {
      ggplot(df, aes(x = Year, y = Emissions, group = Sector, color = Sector)) +
        ...
        scale_y_continuous(
          position="left",
          sec.axis = dup_axis(
            breaks = df %>% filter(Year == max(Year)) %>% pull(Emissions) %>% repel(gap=1),
    ...
    aspect_ratio = 1,
    title = "Agriculture and energy sectors take up the largest portions of greenhouse gas emissions in Nepal.",
...
fig_ch1_GHG_emissions_area <- function() {
  ...
  figure(
    ...
    aspect_ratio = 1,
    title = "Emissions are primarily from agriculture and energy, comprising 47% and 43% respectively, followed by Land-Use Change and Forestry (6.1%), Waste (1.8%), and Industrial Processes (1.7%).",
    subtitle = "Historical Country Greenhouse Gas Emissions (MtCO2e), 1990-2018",
    source = "Source: Climate Watch Historical Country Greenhouse Gas Emissions Data (1990-2018)."
  )
}
...
fig_ch1_GHG_emissions_pie <- function() {
  ...
  df$label <- c("Agriculture\n47.2%", "Energy\n43.3%", "Land-Use Change\nand Forestry\n6.1%", "Waste\n1.8%", "Industrial Processes\n1.7%")
  df$vjust <- c(0.5, 0.5, 0.99, 0.6, -0.1)
  ...
  figure(
    ...
    plot = function(df, style = style_SAR_CCDR()) {
      ...
      theme(legend.position = "none",
            ...
            axis.text.x = element_blank(),
            axis.text.y = element_blank(),
            plot.margin = margin(0,1,-15,1, unit = "mm"))
    },
    aspect_ratio = 1,
    title = "Emissions are primarily from agriculture and energy, comprising 47% and 43% respectively, followed by Land-Use Change and Forestry (6.1%), Waste (1.8%), and Industrial Processes (1.7%).",
