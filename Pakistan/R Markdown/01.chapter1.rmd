  ...
  figure(
    ...
    plot = function(df, style = style_SAR_CCDR()) {
      ggplot(df, aes(x = Year, group = Scenario)) +
        geom_ribbon(aes(ymin = (Reduction+diff)*coeff, ymax = (0+diff)*coeff, fill = factor(Scenario, levels = ordered_scenario)), alpha=0.3) +
    ...
    aspect_ratio = 1,
    subtitle = "GHG emissions (MtCO2e) and Reductions from BAU (%)",
