...
fig_ch1_GHG_emissions_line <- function() {
  ...
  df <- df[-c(1,2),] %>%
    ...
    mutate(Year = as.numeric(Year)) %>%
    mutate(Sector_merged = ifelse(Sector %in% c("Energy", "Agriculture", "Electricity/Heat", "Transportation", "Manufacturing/Construction"),
                                  Sector, 
                                  "Other")) %>%
    dplyr::select(-Sector) %>%
  ...
  figure(
    ...
    aspect_ratio = 1,
    title = "Emissions are primarily from energy and agriculture, comprising 32% and 29% respectively, followed by Electricity/Heat (9.2%), Transportation (8.7%), and Manufacturing/Construction (8.5%).",
    subtitle = "Historical Country Greenhouse Gas Emissions (MtCO2e), 1990-2018",
    source = "Source: Climate Watch Historical Country Greenhouse Gas Emissions Data (1990-2018)."
  )
}
...
fig_ch1_GHG_emissions_pie <- function() {
  ...
  figure(
    ...
    plot = function(df, style = style_SAR_CCDR()) {
      ...
      geom_rect() +
      geom_label(x=4.5, aes(y=labelPosition, label=label), size=2.5, label.size = NA, fill = NA) +
      coord_polar(theta="y") +
      xlim(c(2, 4.5)) +
      scale_fill_manual(values = c(
        "Agriculture" = "#336699", 
