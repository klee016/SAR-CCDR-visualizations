...
fig_ch1_GHG_emissions_line <- function() {
  ...
  df <- df[-c(1,2),] %>%
    ...
    select(Sector, Year, Emissions) %>%
    mutate(Year = as.numeric(Year)) %>%
    mutate(Sector_merged = ifelse(Sector %in% c("Energy", "Agriculture", "Electricity/Heat", "Land-Use Change and Forestry", "Waste"),
                                  Sector, 
                                  "Other")) %>%
    select(-Sector) %>%
  ...
  figure(
    ...
    aspect_ratio = 1,
    title = "Emissions are primarily from agriculture and energy, comprising 29% and 28% respectively, followed by Electricity/Heat (13%), Land-Use Change and Forestry (7.1%), and Waste (6.7%).",
    subtitle = "Historical Country Greenhouse Gas Emissions (MtCO2e), 1990-2018",
    source = "Source: Climate Watch Historical Country Greenhouse Gas Emissions Data (1990-2018)."
  )
}
...
fig_ch1_GHG_emissions_pie <- function() {
  ...
  figure(
    ...
    plot = function(df, style = style_SAR_CCDR()) {
      ...
      geom_rect() +
      geom_label(x=4.4, aes(y=labelPosition, label=label), size=2.5, label.size = NA, fill = NA) +
      coord_polar(theta="y") +
      xlim(c(2, 4.5)) +
      scale_fill_manual(values = c(
          "Agriculture" = "#2F4858", 
...
fig_ch1_GHG_emissions_line_pie <- function() {
  ...
  figure(
    ...
    plot = function(df, style = style_SAR_CCDR()) {
      ...
      df.pie$label <- paste0(df.pie$Sector, "\n", round(df.pie$percentage,1), "%")
      df.pie$label <- c("Agriculture\n28.7%", "Energy\n27.8%", "Electricity/Heat\n12.6%", "Land-Use Change\nand Forestry\n7.1%", "Waste\n6.7%", "Other\n17.1%")
      
      p.pie <- ggplot(df.pie, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=Sector)) +
        geom_rect() +
        geom_label(x=4.5, aes(y=labelPosition, label=label), family=style$family, size=2.5, label.size = NA, fill = NA) +
        coord_polar(theta="y") +
        xlim(c(2, 4.5)) +
      ...
      chart <- gtable_row(
        ...
        height = unit(1, "null"), 
        widths = unit.c(unit(1, "null"), unit(0.0, "in"), unit(1, "null"))
        )
      chart$theme <- style$theme()
      chart
